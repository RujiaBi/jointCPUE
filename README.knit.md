...
where $b_i$ is CPUE, $\beta_{m_i}$ is the month fixed effect used only for
yearly models, and $\sum_m \beta_m = 0$. The standardized index is then
calculated from the population-level latent surface only:
